...
extern cJSON* cJSON_Root;    //json根节点
...
MQTT_SEND_START:
    ...
    xReturn = xQueueReceive( MQTT_Data_Queue,    /* 消息队列的句柄 */
      ...
      if(xReturn == pdTRUE)
      {
        ...
        {
					case 1:
						    cJSON_Data = cJSON_Data_Init(LEDR,1);
					break;
					case 2:
						    cJSON_Data = cJSON_Data_Init(LEDR,0);
					break;
					case 3:
						    cJSON_Data = cJSON_Data_Init(LEDG,1);
					break;
					case 4:
						    cJSON_Data = cJSON_Data_Init(LEDG,0);
					break;
					case 5:
						    cJSON_Data = cJSON_Data_Init(LEDB,1);
					break;
					case 6:
						    cJSON_Data = cJSON_Data_Init(LEDB,0);
					break;
        }
				            //更新数据成功，
				...
				PRINT_DEBUG("%s\n",p);
				//发布消息
				ret = MQTTMsgPublish(MQTT_Socket,(char*)TOPIC1,QOS0,(uint8_t*)p);
				vPortFree(p);  //释放内存 
				p = NULL;
				if(ret >= 0)
				{
